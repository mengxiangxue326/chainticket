...
Create a local configuration file before running the demo:

```sh
cp config.example.yaml config.yaml
```

Fill `config.yaml` with local/demo private keys for the active network. Do not commit `config.yaml`.
